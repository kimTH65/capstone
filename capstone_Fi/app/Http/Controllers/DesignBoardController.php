...
class DesignBoardController extends Controller
{
    public function design_board()
    {
        ...
        $d_pages = DB::table('design_boards')->whereNull('deleted_at')->orderBy('id', 'desc')->paginate(6);
        //-------------- ------- -------------------------------------------------
    ...
    public function design_form(Request $request)
    {
           ...
           $u_pou = $request->input('u_pou'); // 사용 용도
           $u_job = $request->input('u_job'); // 사용자 직업
        ...
        $created_at = $curTime->format("Y-m-d H:i:s");
         ...
         $design_forms = [
             ...
             'design_form_upou' => $u_pou,
             'design_form_ujob' => $u_job,
    ...
    {
        $d_boards = design_form::all();
        session_start();
        if($_SESSION['job'] == 'designer')
        {
            $form_pages = DB::table('design_forms')->where('design_form_designer',$_SESSION['id'])->orderBy('id', 'desc')->paginate(5);
        }
        else
        {
             $form_pages = DB::table('design_forms')->where('design_form_applicant',$_SESSION['id'])->paginate(5);
        }
